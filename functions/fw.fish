...
function fw
    ...
    set SEARCH_RESULT (sk --ansi -i -c 'rg --line-number --ignore-case "{}" ~/wiki' --delimiter=':' --preview 'bat {1} --color=always --line-range=:500 --decorations=always --highlight-line={2} --theme=base16' --preview-window position:$DIRECTION +{2}-/2 | awk -F':' '{print $1" +"$2}')
